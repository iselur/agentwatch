...
`_patched_files` exists twice — here and in `agentlog/parser.py` — because
the two tools read the same Codex log for different purposes and the family
does not import across packages.  Two copies of one scanner is a fact of the
layout; two copies that disagree is a bug, and it was one.
...
agentwatch's copy.  A change to either scanner that is not made to both breaks
...
from agentwatch.events import _patched_files  # noqa: E402
...
class TestThePatchEnvelopeScannerBothToolsShare(unittest.TestCase):
    ...
    def test_an_envelope_inside_a_javascript_string_is_still_found(self):
        ...
        self.assertEqual(_patched_files(EMBEDDED_IN_JS), ["src/app.py"])
