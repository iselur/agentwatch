...
A flag nobody can discover is a flag nobody uses.  agentwatch grew an `--interval` and a `--no-color`,
and the README never mentioned either of them: it worked, it was tested, and the
...
command line belonging to agentwatch itself.  READMEs discuss other programs — the
...
it as a flag agentwatch has lost.
"""
...
from agentwatch.cli import build_parser as build_parser
...
FOREIGN = {"git", "pip", "python3", "claude", "codex", "stillworks", "cd", "unedit", "agentdiff", "agentlog"}
