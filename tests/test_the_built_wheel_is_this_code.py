...
agree, and in unedit it passed the whole time `unedit --version` was printing
0.1.3 to everyone who installed 0.1.4 from PyPI.  It passed because it was
right: the *source* agreed with itself.  What went wrong was one step later.
...
agentwatch has not been published yet, so this is here before the mistake
rather than after it — the wheel in dist/ is the one the upload loop is
waiting on a rate limit to send.
...
# "agentwatch-0.1.0-py3-none-any.whl", and distribution names are not
# always the import name elsewhere in the family.  Sdists are spelled differently
# enough to need their own — "agentwatch-0.1.0.tar.gz" has no tag after the
...
class TestTheBuiltWheelIsThisCode(unittest.TestCase):
    ...
    def test_the_wheel_carries_the_version_this_tree_says(self):
        ...
        for path in self.built:
            ...
            self.assertEqual(
                found.group(1), agentwatch.__version__,
